    ...
    setInterval(label, addMethod, done) {
      ...
      const executeInterval = () => {
        ...
        try {
          ...
          server.log(['hapi-method-scheduler', 'error'], { method: scheduleRequest.method, error: e });
          onEnd(e, label);
        }
      }
      method.executingSchedule = later.setInterval(executeInterval, method.interval);
